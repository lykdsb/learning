...
## 内存回收策略
java中的自动内存管理最终可以归结为自动化地解决了两个问题
* 给对象分配内存
* 回收分配出去的内存

垃圾回收就是如何回收分配出去的内存，因此还需要了解内存的分配方式

* MinorGC:清理年轻代
* MajorGC:清理老年代
* FullGC:清理整个堆

1. 对象优先在Eden进行分配，如果Eden没有足够的空间，虚拟机将会发起一次MinorGC
2. 大对象直接进入老年代：需要连续内存空间的java对象，一般是直接进入老年代的，因为这样可以避免在Eden区以及两个Survivor区之间发生大量的内存拷贝
3. 长期存活的对象进入老年代（默认是进行15次MinorGC之后）
4. 动态对象年龄判定：**如果在Survivor空间中相同年龄的虽有对象大小的总和大于Survivor空间的一半，年龄大于或者等于该年龄的对象就可以直接进入老年代**
5. 空间分配担保：发生MinorGC时，虚拟机会检测之前每一次晋升到老年代的平均大小是否大于老年代的剩余空间大小，如果大于，则改为进行一次FullGC，如果小于，则查看是否允许担保失败，如果允许，则智慧进行MinorGC，否则进行FullGC
